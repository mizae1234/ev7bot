...
function SparePartsAuditDashboard() {
  ...
  // Fetch master locations from DB (using existing vehicle audit locations or general ones)
  const fetchLocations = async () => {
    try {
      const res = await fetch('/api/audit/locations') // Can reuse the same locations
      if (res.ok) {
        const data = await res.json()
  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/spare-parts/audit/${data.auditSessionId}`)
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-teal-950 to-slate-900 text-slate-100 font-sans pb-16">
      {/* Premium Header */}
      <div className="sticky top-0 z-30 bg-slate-900/80 backdrop-blur-xl border-b border-teal-500/20 shadow-lg">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold bg-gradient-to-r from-emerald-400 to-teal-300 bg-clip-text text-transparent flex items-center gap-2">
              📦 Spare Parts Audit
            </h1>
            <p className="text-xs text-teal-300 mt-0.5">ระบบตรวจนับสต็อกอะไหล่</p>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white font-bold text-sm px-4 py-2 rounded-xl transition duration-200 shadow-md hover:shadow-lg flex items-center gap-1.5"
          >
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-800/40 border border-teal-500/10 rounded-2xl p-4 text-center backdrop-blur-sm">
        ...
        {/* Sessions List */}
        <div>
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">ประวัติการทำ Audit อะไหล่</h2>
          ...
          {loading ? (
            ...
            <div className="bg-slate-800/20 border border-slate-800 rounded-2xl py-16 text-center text-slate-400">
              ...
              <div className="text-sm font-medium">ยังไม่มีประวัติการทำ Spare Parts Audit</div>
              <button
                onClick={() => setIsModalOpen(true)}
                className="mt-4 text-emerald-400 hover:text-emerald-300 text-xs font-bold underline"
              >
            ...
            <div className="space-y-4">
              {sessions.map((session) => (
                <div
                  ...
                  onClick={() => router.push(`/spare-parts/audit/${session.AuditSessionID}`)}
                  className="bg-slate-800/30 hover:bg-slate-800/50 border border-teal-500/10 hover:border-emerald-500/30 rounded-2xl p-5 transition duration-200 cursor-pointer shadow-sm hover:shadow-md backdrop-blur-sm space-y-3"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="text-lg font-black text-slate-200">{session.CheckedCount} ชิ้น</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-md bg-slate-900 border border-teal-500/20 rounded-2xl p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center pb-2 border-b border-slate-800">
              <h3 className="text-lg font-bold text-slate-100">📦 สร้างรอบนับสต็อกใหม่</h3>
              <button
            ...
            <form onSubmit={handleCreateSession} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-400 transition"
                  required
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 block">สถานที่ / คลังสินค้า</label>
                <select
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-400 transition font-medium"
                  required
                >
              ...
              {location === 'custom' && (
                <div className="space-y-1 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <input
                    type="text"
                    placeholder="เช่น คลัง 1 ชั้น 2"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-emerald-400 transition h-16 resize-none"
                />
              </div>
              ...
              <div className="pt-2 flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white font-bold text-sm py-2.5 rounded-xl transition disabled:opacity-50"
                >
...
export default function SparePartsAuditPage() {
  return (
    <AuthGuard>
      <SparePartsAuditDashboard />
    </AuthGuard>
  )
}
