...
function ClaimLogsContent() {
  ...
  const [summary, setSummary] = useState({ pending: 0, processed: 0, cancelled: 0, total: 0 })
  ...
  const fetchClaims = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/claim-logs?${params.toString()}`)
      if (!res.ok) {
        if (res.status === 401) {
  ...
  const handleAction = async (id: number, action: 'process' | 'cancel') => {
    if (action === 'cancel' && !confirm('คุณต้องการยกเลิกการแจ้งเคลมซ่อมรายการนี้ใช่หรือไม่?')) return
    setActionLoading(id)
    try {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'PROCESSED':
        return (
          <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-450 border border-emerald-500/20">
            ✅ ดำเนินการแล้ว
          </span>
        )
      default:
      ...
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        ...
        >
          <div className="text-center mb-6">
            ...
            <h1 className="text-xl font-bold text-zinc-100">ระบบตรวจจับแจ้งซ่อม (Auto Claims)</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 p-4 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] text-amber-555 font-bold uppercase tracking-wider">⏳ รอดำเนินการ</span>
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/10 p-4 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] text-emerald-500/80 font-bold uppercase tracking-wider">✅ ดำเนินการแล้ว</span>
            <span className="text-2xl font-extrabold text-emerald-400 mt-2">{summary.processed}</span>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 border border-dashed border-zinc-850 rounded-2xl text-zinc-500 text-sm">
            ไม่พบรายการแจ้งซ่อมใดๆ ในระบบขณะนี้
          </div>
        ) : (
          <div className="space-y-4">
            {claims.map((claim) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3 pb-4 border-b border-zinc-900/60">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2.5">
                      <span className="text-sm font-bold text-zinc-100"># {claim.id}</span>
                      
                      {claim.vehicleRef ? (
                        <a
                          href={`/vehicle/${claim.vehicleRef}`}
                          className="px-2 py-0.5 rounded-lg text-xs font-bold bg-indigo-500/15 text-indigo-400 border border-indigo-500/25 hover:bg-indigo-500/25 transition-colors"
                        >
                          🚗 {claim.vehicleRef}
                        </a>
                      ) : (
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-zinc-500">
                      <span>👤 ผู้รายงาน: <span className="text-zinc-300 font-semibold">{claim.displayName || 'ไม่ระบุชื่อ'}</span></span>
                      {claim.lineUserId && (
                        <>
                          <span>•</span>
                          <span className="font-mono text-[10px] text-zinc-650 bg-zinc-950/45 px-1.5 py-0.5 rounded">ID: {claim.lineUserId}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="text-right text-[11px] text-zinc-500 space-y-0.5">
                    <div>📅 บันทึกเมื่อ: {formatDateTime(claim.createdAt)}</div>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
                  <div>
                    <span className="block text-[10px] text-emerald-450 font-bold mb-1.5 uppercase tracking-wider">🛠️ ข้อสรุปจาก AI (Extracted Detail)</span>
                    <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3.5 text-xs text-emerald-300 font-medium leading-relaxed">
                      {claim.extractedDetail || 'ไม่ได้ระบุรายละเอียดปัญหา'}
                    </div>
                  </div>

                  <div>
                    <span className="block text-[10px] text-zinc-500 font-bold mb-1.5 uppercase tracking-wider">💬 ข้อความแชทดิบ (Raw Message)</span>
                    <div className="bg-zinc-950/40 border border-zinc-900 rounded-xl p-3.5 text-xs text-zinc-400 leading-relaxed font-mono whitespace-pre-wrap">
                      {claim.message}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-zinc-900/40 text-[11px]">
                  <div className="text-zinc-500">
                    📂 แหล่งที่มา: {' '}
                    <span className="font-semibold text-zinc-400 capitalize">LINE Group</span>
                    {claim.groupId && (
                      <span className="font-mono text-[10px] bg-zinc-950/30 px-1.5 py-0.5 rounded ml-1 text-zinc-500">GID: {claim.groupId}</span>
                    )}
                  </div>
                  ...
                  {claim.status === 'PENDING' && (
                    <div className="flex gap-2">
                      ...
                      >
                        {actionLoading === claim.id ? 'กำลังบันทึก...' : '✅ ดำเนินการแล้ว'}
                      </button>
                    </div>
                  )}
                </div>
...
export default function ClaimLogsPage() {
  return (
    <AuthGuard>
      <ClaimLogsContent />
    </AuthGuard>
