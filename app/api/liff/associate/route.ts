  ...
  try {
    ...
    if (action === 'link') {
      ...
      try {
        ...
        console.error('[Liff Associate SQL Insert Error]', sqlErr)
        return NextResponse.json({
          error: `ไม่สามารถบันทึกบัญชีผู้ใช้ใหม่ลงฐานข้อมูลได้: ${sqlErr.message}`
        }, { status: 500 })
      }
    } else {
    ...
    // Upsert into PostgreSQL line_registrations
