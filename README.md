...
### 8. 🚧 บันทึกรถเข้า-ออกลาน (Gate Log Monitor) (`/gate-monitor`)
- ตรวจจับและบันทึกข้อมูลรถยนต์เข้า-ออกลานจอดอัตโนมัติจากข้อความ LINE ของ รปภ (AI-powered parsing via Gemini 3.1 Flash Lite)
- ควบคุมการเปิด/ปิดรายกลุ่ม LINE ด้วยคำสั่ง `butter เปิดบันทึกเข้าออก` / `butter ปิดบันทึกเข้าออก` (ในกลุ่มที่เปิดจะปิดการตอบทั่วไปของ Butter เพื่อความเงียบ)
- ระบบ Normalization ทะเบียนรถ (`ทอ-1234`) และระบบจัดการรถใหม่ไม่มีทะเบียน (Single-record batch + `QuantityIn`/`QuantityOut` พร้อม FIFO out matching)
- แดชบอร์ดสรุปสถิติรถเข้า, รถออก, รถที่ยังจอดอยู่ในลานแบบ Real-time พร้อมส่งออกไฟล์ Excel
